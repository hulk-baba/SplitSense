...
<?php
	...
	$myfile = fopen("tmp_multi.txt","w") or die("Unable to open file");
	$i=0;
	foreach ($_POST as $key => $value) {
 		if($i>=2 && $i%2==0 && $key != "cash"){
 			$str =  substr($key, 0, -11) . ' ' . $value . "\n";
 			fwrite($myfile, $str) ; 
 			// echo nl2br("\r\n");
 		}
 		else if($i>=2 && $i%2!=0 && $key !="cash"){
 			$str =  $key . ' ' . $value . "\n";
 			fwrite($myfile, $str) ; 
 		}
 		$i++;
	}
	exec('/var/www/html/ProjectYelp/edge_multi');
