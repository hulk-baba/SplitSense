...
void total_calculation(){
	for(int i=1;i<100;i++){
		if(tree[i].net || tree[i].total_gets || tree[i].total_owed){
			outfile<<mymap[i]<<" is owed a total amount of ₹ "<<tree[i].total_owed;opn;
			outfile<<mymap[i]<<" gets back a total amount of ₹ "<<tree[i].total_gets;opn;
			outfile<<mymap[i]<<" has net balance of amount in ('-' indicates one is in debt )   => "<<" ₹ "<<cost[i];opn;
			tr(tree[i].gets,it){
				outfile<<mymap[i]<<" gets back from "<<mymap[it->first]<<" an amount of ₹ "<<it->second;
				opn;
			}
			tr(tree[i].owes,it){
				outfile<<mymap[i]<<" owes to "<<mymap[it->first]<<" an amount of ₹ "<<it->second;
				opn;
			}
		}
		opn;
...
void simplify_debt(){
	while(1){
		...
		int mini = min(abs(maxc.first) , abs(minc.first));
		if(mini == abs(minc.first) && mini != abs(maxc.first)){
			outfile<<mymap[minc.second]<<" has to pay "<<mymap[maxc.second]<<" an amount of ₹ "<<mini;
			opn;
			minq.pop();
			maxq.pop();
...
int main(){
	...
	outfile<<"This is the Simplified Debt Information :-\n\n";
	simplify_debt();
	opn;
	outfile<<"This is the Detailed Information for each User :-\n\n";
	total_calculation();
